-- COMPREHENSIVE QUIZ COMPLETION DATA RECOVERY
-- This migration fixes the system-wide quiz completion issue by:
-- 1. Creating a function to identify users who passed quizzes but don't have completion records
-- 2. Backfilling missing user_unit_progress records
-- 3. Recalculating course progress for affected users

-- Function to identify and fix missing quiz completion records
...
BEGIN
  ...
  INSERT INTO public.user_management_audit (
    ...
    auth.uid(),
    'System-wide fix for missing quiz completion records',
  ...
  FOR rec IN 
    WITH quiz_passes AS (
      SELECT DISTINCT
        ...
        p.email,
        ROW_NUMBER() OVER (PARTITION BY qa.user_id, qa.quiz_id ORDER BY qa.score DESC, qa.created_at DESC) as rn
    ...
    missing_completions AS (
      SELECT 
        ...
        qp.email
      FROM quiz_passes qp
      WHERE qp.rn = 1  -- Only the best attempt per user per quiz
        AND NOT EXISTS (
  ...
  RETURN QUERY SELECT 
    ...
    jsonb_build_object(
      'affected_users', v_affected_users,
      'audit_id', v_audit_id,
      'operation', 'mass_quiz_completion_recovery'
    );
END;
$$;

-- Function to analyze the scope of the issue before fixing
CREATE OR REPLACE FUNCTION public.analyze_missing_quiz_completions()
RETURNS TABLE(
  total_passed_quizzes BIGINT,
  missing_completion_records BIGINT,
  affected_users BIGINT,
  affected_courses BIGINT,
  sample_affected_users JSONB
)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
