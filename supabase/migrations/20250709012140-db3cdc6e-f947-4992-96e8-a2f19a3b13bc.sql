-- Drop existing conflicting function and create optimized database functions for centralized progress calculations

-- Drop existing function that conflicts
DROP FUNCTION IF EXISTS public.get_team_progress_summary(uuid);
...
-- Create function for team progress aggregation with new name
CREATE OR REPLACE FUNCTION public.get_optimized_team_progress(p_team_id UUID)
RETURNS TABLE(
  user_id UUID,
  user_email TEXT,
  user_name TEXT,
